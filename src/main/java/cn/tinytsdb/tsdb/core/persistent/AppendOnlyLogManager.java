...
public class AppendOnlyLogManager implements Initializer, Persistently , Closer {
    ...
    public void init() {
        ...
        try {
            currentLogOs = storeHandler.getFileAppendStream(AOL_FILE);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public AOLog[] recoverLog() {
        try {
            if(storeHandler.fileExisted(AOL_FILE)) {
                InputStream inputStream = storeHandler.getFileInputStream(AOL_FILE);
    ...
    public boolean resetLogBuf() {
        if(persistLogIdx()) {
            try {
                currentLogOs = storeHandler.getFileOutputStream(AOL_FILE);
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private boolean persistLogIdx() {
        try (OutputStream outputStream = storeHandler.getFileOutputStream(LOG_IDX_FILE)){
