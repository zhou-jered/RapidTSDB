...
public class TSBlockMeta implements Serializable {
    ...
    private final static int SERIES_LEN = 8 + 4 + 4 + 4 + 16;
    ...
    public byte[] series() {
        ...
        ByteAppender byteAppender = (pos, len, val, result) -> {
            long value = val;
        ...
        int idx = 8;
        byteAppender.append(idx, 8, baseTime, seriesData);
        idx += 4;
        byteAppender.append(idx, 4, dpsSize, seriesData);
        idx += 4;
        byteAppender.append(idx, 4, timeBitsLen, seriesData);
        idx += 4;
        byteAppender.append(idx, 4, valuesBitsLen, seriesData);
    ...
    public static TSBlockMeta fromSeries(byte[] series, int position) {
        ...
        blockMeta.baseTime = Longs.fromBytes(series[position], series[position+1],series[position+2],series[position+3],
                series[position+4],series[position+5],series[position+6],series[position+7]);
        position+=8;
